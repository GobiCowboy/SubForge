...
| ISSUE-022 | P1 | 待本地验收 | 客户端最大字数分段在语义中间硬切 | 公共分段器现将字数作为目标值，按标点、停顿和词组边界回退；真实 ASR 词元不再被二次插值拆开 |
| ISSUE-023 | P1 | 待在线验收 | 官方智能字幕进度未区分 ASR 转写与 AI 校对 | 已映射服务端 `proofreading` 独立状态，完成结果还必须验证 `proofreadingApplied=true`，不能提前标记校对完成 |
| ISSUE-024 | P0 | 已修复 | StoreKit 成交后若 Apple 通知漏发，钱包额度永久不到账 | 客户端购买后和App启动时上报未确认transaction ID，Billing向Apple反查验签并幂等履约；Billing另外每5分钟拉取失败通知历史。真实Sandbox已恢复漏单`H5Q5…`；`oQQ…`经Apple历史确认没有交易，未发生扣款 |
| ISSUE-025 | P1 | 待在线验收 | 流水线取消语义不完整 | 客户端取消会请求鉴权取消接口；服务端终止后续校对与结算、尝试取消仍在排队的 ASR，并返还预留额度 |
| ISSUE-026 | P0 | 待在线验收 | 官方智能字幕丢失字词级时间戳 | 服务端现完整交付所有 transcript 的 `words`，客户端优先按真实词起止时间分段；仅上游无词时间时才显式降级估算 |
| ISSUE-027 | P1 | 待本地验收 | 自定义云端 ASR 未稳定复用字词级公共分段 | DashScope 与 Whisper-compatible 解析均优先归一为 `SubtitleWord` 并进入 `TimedSubtitleSegmenter`，无词时间路径记录明确降级日志 |
| ISSUE-028 | P0 | 待在线验收 | 官方 AI 校对完成状态不可验证 | 服务端不再静默跳过校对，并在真实执行后返回 `proofreadingApplied=true`；客户端未验证该字段时拒绝进入成功编辑页 |
| ISSUE-029 | P0 | 已修复 / 待口播验收 | 字幕按最大字数硬切、语气词被并到上一句且跨ASR segment合并 | 公共分段器改为source segment硬边界；逗号等一级标点优先，顿号/冒号只在超长时回退；不足3个有效字的片段优先向后合并 |
| ISSUE-030 | P1 | 已实现 / 待在线验收 | ASR不支持热词且官方服务需要收到本地术语 | 转写前逐视频热词与通用设置固定热词可独立启用、合并去重，由AI根据上下文纠正；客户端和Model API已透传受限校对提示词，固定系统约束不可覆盖 |
| ISSUE-031 | P0 | 已修复 / 待TestFlight验收 | 官方智能字幕提交失败只显示HTTP状态且任务暂时无法再次操作 | 客户端现解析服务端稳定错误与额度返还标记，展示上游暂时不可用原因，并引导用户稍后点击顶部任务记录继续；未新增独立重试或取消按钮 |
| ISSUE-032 | P1 | 已解决 | FCPXML 导入后偶发显示默认“标题”占位文字 | 根因是动态 Custom 标题模板叠加 offset、duration 独立舍入，造成故事线一帧重叠或空洞；现由 `FCPXMLTimelinePlanner` 统一生成连续整数帧区间，改用静态 Basic Title，并经用户在 Final Cut Pro 中实际导出播放验收 |
| ISSUE-033 | P1 | 已修复 / 待用户复测 | 播放暂停后字幕光标跳到末尾且无法继续编辑 | 暂停切换到 `NSTextView` 时，首次 focus 产生的默认选区会覆盖暂停位置；现保存暂停瞬间的字幕与 UTF-16 光标，在 first responder 建立后再次恢复，并保持文本框可编辑 |
