...
| ISSUE-026 | P0 | 待修复 | 官方智能字幕丢失字词级时间戳 | 官方服务虽向 DashScope 请求 `enable_words=true`，但服务端结果映射只保留句子起止时间，客户端只能按字符估算句内时间，造成字幕切换滞后；服务端和客户端契约必须完整传递真实字词时间戳，并由公共分段器消费 |
| ISSUE-027 | P1 | 待修复 | 自定义云端 ASR 未稳定复用字词级公共分段 | DashScope Filetrans 与 Whisper-compatible 返回结构不同，当前解析路径未统一保证优先读取 word timestamps，缺失时也缺少明确降级标记；需要统一解析为 `SubtitleWord` 后进入 `TimedSubtitleSegmenter` |
| ISSUE-028 | P0 | 待修复 | 官方 AI 校对完成状态不可验证 | 服务端缺少校对密钥时会原样返回 ASR 字幕，客户端仍标记“智能校对已完成”；完成结果必须携带真实校对执行状态，未执行或失败不得伪装成完成，进度阶段必须与服务端实际操作对应 |
