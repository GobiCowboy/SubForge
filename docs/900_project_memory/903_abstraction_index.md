...
| A-011 | 字幕文本输入与光标定位 | 304 | 已抽象 | `SubtitleTextEditor` 统一表格与 Inspector 的 AppKit 文本输入，向编辑模型回传 UTF-16 光标 / 选区；暂停恢复时在 first responder 建立后再次确认初始位置；分割规则集中在 `SubtitleSplitService` |
| A-012 | 播放时间到字幕光标映射 | 304 | 已抽象 | `SubtitleSplitService.caretOffset` 统一处理词级时间、词内插值、字符边界和无词级时间的比例回退；播放展示和暂停编辑共享同一映射结果 |
