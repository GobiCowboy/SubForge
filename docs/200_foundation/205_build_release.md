...
- 2026-09-01 已成功上传并正式发布 App Store Connect 构建：版本 `1.0.11`，Build `20260901232611`，Delivery UUID `c0b4e8a8-d265-4244-a9a8-c191334d1145`；源码提交 `7b6fd7f`。本版本包含字幕分割、编辑快捷键、播放光标跟随和暂停后保留编辑光标；App Store 通过后已将 `manifest.json` 的正式 `latestVersion` 更新为 `1.0.11`
...
所有项目通用的“创建任务分支 → 合并前验证 → 合并后核对 → 清理本地和远程分支”流程，以知识库权威文档 [`108_AI开发工作流.md`](../../../../data/obsidian/一以贯之/知识库/claude/108_AI开发工作流.md) 的「通用 Git 分支生命周期」为准。本项目额外要求：发布相关改动必须通过 Swift 测试、签名/打包检查和发布内容哈希核对后再合并。
