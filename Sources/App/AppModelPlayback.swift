...
extension AppModel {
  ...
  private func pausePlaybackAndEnterEditing() {
    let pauseTime = currentTime
    syncSelectionToCurrentTime()
    let pauseSegmentID = selectedSegmentID ?? segments.first?.id

    stopPlayback()

    if selectedSegmentID == nil {
      selectedSegmentID = pauseSegmentID
    }

    if let segment = segments.first(where: { $0.id == pauseSegmentID }) ?? selectedSegment {
      subtitleTextCaret = SubtitleTextCaret(
        segmentID: segment.id,
        utf16Offset: SubtitleSplitService.caretOffset(at: pauseTime, in: segment),
        selectionLength: 0
      )
    }
    ...
    beginEditingSelectedSubtitle(surface: .table)
  }
