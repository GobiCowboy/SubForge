...
struct ShortcutGuidePanel: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: compact ? 12 : 16) {
      ...
      let items = compact ? compactShortcutItems : editorShortcutItems
      VStack(alignment: .leading, spacing: compact ? 11 : 0) {
...
private struct ShortcutGuideRow: View {
  ...
  var body: some View {
    HStack(alignment: .center, spacing: compact ? 12 : 28) {
      ShortcutKeyGroup(keys: item.displayKeys(compact: compact), compact: compact)
        .frame(width: compact ? 120 : 220, alignment: .leading)
    ...
    HStack(spacing: 5) {
      ForEach(Array(keys.enumerated()), id: \.offset) { index, key in
        ...
        KeyboardShortcutBadge(
          text: key,
          compact: compact,
          fontSize: compact ? nil : 14,
          fontWeight: compact ? .semibold : .bold,
          height: compact ? nil : 32
        )
      }
    }
  }
