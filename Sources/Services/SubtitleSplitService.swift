...
enum SubtitleSplitService {
  ...
  static func caretOffset(
    ...
    guard !segment.text.isEmpty else { return 0 }

    let clampedTime = min(max(time, segment.start), segment.end)
    let spans = wordSpans(in: segment)
    guard !spans.isEmpty else {
      let ratio = (clampedTime - segment.start) / max(segment.end - segment.start, 0.1)
      let target = Int((Double(segment.text.utf16.count) * ratio).rounded(.down))
      return characterBoundary(in: segment.text, nearestTo: target)
    }

    for span in spans {
      let start = span.range.lowerBound.utf16Offset(in: segment.text)
      let end = span.range.upperBound.utf16Offset(in: segment.text)

      if clampedTime < span.word.start {
        return start
      }
      if clampedTime < span.word.end {
        let duration = max(span.word.end - span.word.start, 0.01)
        let ratio = (clampedTime - span.word.start) / duration
        let target = start + Int(Double(end - start) * ratio)
        return characterBoundary(in: segment.text, nearestTo: target)
      }
    }

    return segment.text.utf16.count
  }
  ...
  static func splitAtPlayhead(
    ...
    guard time > segment.start + minimumDuration,
      time < segment.end - minimumDuration
    else {
  ...
  private static func timeForCaret(
    ...
    if let span = spans.first(where: {
      $0.range.upperBound.utf16Offset(in: segment.text) == utf16Offset
    }) {
  ...
  private static func wordSpans(in segment: SubtitleSegment) -> [WordSpan] {
    ...
    return words.compactMap { word in
      ...
      guard !normalized.isEmpty,
        ...
        let range = segment.text.range(of: normalized, range: searchStart..<segment.text.endIndex)
      else {
        return nil
      }
  ...
  private static func nearestCharacterBoundary(in text: String, aroundUTF16Offset offset: Int)
    -> Int
  {
  ...
  private static func characterBoundary(in text: String, nearestTo offset: Int) -> Int {
    guard !text.isEmpty else { return 0 }
    let clamped = min(max(offset, 0), text.utf16.count)
    if clamped == 0 || clamped == text.utf16.count { return clamped }
    return nearestCharacterBoundary(in: text, aroundUTF16Offset: clamped)
  }
}
