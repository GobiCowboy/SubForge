...
SubForge 是一个面向 Final Cut Pro（FCP）的字幕工作流应用，基于 FTP 进行文件交换：可监听 FTP 上 FCP 导出的文件，也能将生成的字幕导入回 FTP，衔接剪辑工作流。重点是把 **监听 → 转写 → 编辑 → 导出** 这条路径做短、做稳、做清楚。

## 使用流程

打开应用后先进入「设置」页面（本应用的主要操作都在设置内完成）。

1. **目录监听（FTP）**：在「设置」中进入「目录监听」页面，配置 FTP 连接与监听目录。应用会自动监听 FTP 上 FCP 导出的文件并自动进行转写。
2. **将字幕导入 FTP**：在「设置 → 导出」页配置 FTP 目标，将生成的字幕导入回 FTP，供 Final Cut Pro 取用。
3. **配置校对与转写**：按需配置校对（AI 文本修正）与转写（模型、语言等）参数，并滑到页面底部进行验证，确认配置生效。
4. **导出并回灌样式**：导出字幕到 FTP；在 Final Cut Pro 中确认后，将对应参数配置回「设置 → 基本样式」，使后续导出保持一致。
...
- **导出与交付**：输出 SRT、FCPXML，并通过 FTP 衔接 Final Cut Pro 工作流。
- **目录监听**：监听 FTP 目录，自动识别 FCP 导出的文件并转写。
- **菜单栏常驻**：支持菜单栏常驻模式。
