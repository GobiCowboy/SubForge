...
1. **目录监听**：在「设置」中进入「目录监听」页面，选择你制作视频的根目录。应用会自动识别 Final Cut Pro 导出的音频并自动进行转写。
2. **配置导出到 FCP**：在「设置 → 导出」页勾选「导出到 FCP」，之后导出字幕可直接唤起 Final Cut Pro 并导入，无需手动搬运文件。
3. **配置校对与转写**：按需配置校对（AI 文本修正）与转写（模型、语言等）参数，并滑到页面底部进行验证，确认配置生效。
4. **导出 FCPXML 并回灌样式**：在应用中导出 FCPXML；到 Final Cut Pro 中修改对应参数后，将参数配置回「设置 → 基本样式」，使后续导出保持一致。
